...
public class lessons250121 {
    ...
    public static int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
        List<int[]> sortedList = sorting(new ArrayList<>(Arrays.asList(data)), ext, val_ext, sort_by);
    ...
    public static List<int[]> sorting(ArrayList<int[]> data, String ext, int val_ext, String sort_by){

        // data에서 ext 값이 val_ext보다 작은 데이터만 뽑은 후,
        // sort_by에 해당하는 값을 기준으로 오름차순으로 정렬하여 return 하도록 solution 함수를 완성해 주세요.
        // 단, 조건을 만족하는 데이터는 항상 한 개 이상 존재합니다.

        List<int[]> firstFilter = new ArrayList<>();
        if (Objects.equals(ext, "code")) {
            firstFilter = data.stream().filter(a -> a[0] < val_ext).collect(Collectors.toCollection(ArrayList::new));
        } else if (Objects.equals(ext, "date")) {
            firstFilter = data.stream().filter(a -> a[1] < val_ext).collect(Collectors.toCollection(ArrayList::new));
        } else if (Objects.equals(ext, "maximum")) {
            firstFilter = data.stream().filter(a -> a[2] < val_ext).collect(Collectors.toCollection(ArrayList::new));
        } else if (Objects.equals(ext, "remain")) {
            firstFilter = data.stream().filter(a -> a[3] < val_ext).collect(Collectors.toCollection(ArrayList::new));
        }

        if (Objects.equals(sort_by, "code")) {
            firstFilter.sort(Comparator.comparingInt(d -> d[0]));
        } else if (Objects.equals(sort_by, "date")) {
            firstFilter.sort(Comparator.comparingInt(d -> d[1]));
        } else if (Objects.equals(sort_by, "maximum")) {
            firstFilter.sort(Comparator.comparingInt(d -> d[2]));
        } else if (Objects.equals(sort_by, "remain")) {
            firstFilter.sort(Comparator.comparingInt(d -> d[3]));
        }
