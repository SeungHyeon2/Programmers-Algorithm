...
public class gameMapShortcut_1844 {
  ...
  public static void dfs(int xPos, int yPos, int distance, int[][] maps) {

    int n = maps.length;
    int m = maps[0].length;

    if (xPos >= 0 && xPos < n && yPos >= 0 && yPos < m && maps[xPos][yPos] == 1
        && !visited[xPos][yPos]) {
      ...
      int[][] wayToMove = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};

      for (int[] way : wayToMove) {
        int newXPos = xPos + way[0];
        int newYPos = yPos + way[1];
        dfs(newXPos, newYPos, distance + 1, maps);
      }
