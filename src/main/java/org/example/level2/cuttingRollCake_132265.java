...
public class cuttingRollCake_132265 {
...
//        int[] topping = {1, 2, 1, 3, 1, 4, 1, 2};
    ...
    public static int solution(int[] topping) {
        ...
        // 형(왼쪽)의 토핑 개수를 저장할 HashSet
        Set<Integer> person1Set = new HashSet<>();
        // 동생(오른쪽)의 토핑 개수를 저장할 HashMap
        Map<Integer, Integer> person2Map = new HashMap<>();

        // 먼저 모든 토핑을 동생이 다 가지고 있다고 가정하고, HashMap 초기화
        for (int t : topping) {
            person2Map.put(t, person2Map.getOrDefault(t, 0) + 1);
        }

        // 한 개씩 형에게 주면서 비교
        for (int i = 0; i < topping.length - 1; i++) {
            person1Set.add(topping[i]); // 형에게 토핑 추가
            person2Map.put(topping[i], person2Map.get(topping[i]) - 1);

            // 동생이 해당 토핑을 더 이상 가지지 않으면 Map에서 제거
            if (person2Map.get(topping[i]) == 0) {
                person2Map.remove(topping[i]);
            }

            // 형과 동생의 토핑 종류 개수가 같으면 answer 증가
            if (person1Set.size() == person2Map.size()) {
                answer++;
            }
        }
